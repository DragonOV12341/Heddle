...
/*!
 * \brief Enable Heddle consumer-side scheduling before FineGrainedWS.
 *
 * When True, the Heddle SMT scheduler reorders consumer statements
 * in pipeline loops for optimal barrier placement by FineGrainedWS.
 *
 * kEnableHeddleConsumerSchedule = "tl.enable_heddle_consumer_schedule"
 */
static constexpr const char *kEnableHeddleConsumerSchedule =
    "tl.enable_heddle_consumer_schedule";

/*!
 * \brief Enable the full Heddle AutoMixed pipeline annotation pass.
 *
 * When True, uses Heddle to compute pipeline stage/order/group annotations
 * automatically for T.Pipelined loops via SMT scheduling.
 *
 * kEnableAutoTLPipelineSMT = "tl.enable_auto_tl_pipeline_smt"
 */
static constexpr const char *kEnableAutoTLPipelineSMT =
    "tl.enable_auto_tl_pipeline_smt";

/*!
 * \brief Use architecture-aware latency model in Heddle consumer scheduling.
 *
 * kHeddleUsePreciseLatency = "tl.heddle_use_precise_latency"
 */
static constexpr const char *kHeddleUsePreciseLatency =
    "tl.heddle_use_precise_latency";

/*!
 * \brief Enable ALAP+slack-based scheduling in Heddle consumer schedule.
 *
 * kHeddleUseAlapPriority = "tl.heddle_use_alap_priority"
 */
static constexpr const char *kHeddleUseAlapPriority =
    "tl.heddle_use_alap_priority";

/*!
 * \brief Enable buffer-span-aware priority in Heddle consumer schedule.
 *
 * kHeddleBufferSpanAware = "tl.heddle_buffer_span_aware"
 */
static constexpr const char *kHeddleBufferSpanAware =
    "tl.heddle_buffer_span_aware";

/*!
 * \brief Relax producer-boundary constraints to per-buffer ordering.
 *
 * kHeddleRelaxProducerBoundary = "tl.heddle_relax_producer_boundary"
 */
static constexpr const char *kHeddleUsePhaseB =
    "tl.heddle_use_phase_b";

static constexpr const char *kHeddleRelaxProducerBoundary =
    "tl.heddle_relax_producer_boundary";

/*!
 * \brief Producer thread extent for FineGrainedWS warp specialization.
 *
 * For TMA-only producers, only 1 thread issues TMA; the remaining
 * producer threads are idle.  Reducing from the default 128 to 32
 * (1 warp) saves register file pressure (96 fewer threads × regs/thread)
 * and reduces warp scheduling overhead.
 *
 * The value MUST be a multiple of 32 (warp size) and >= 32.
 * When the producer loop contains lowered SIMT copies, this config
 * is overridden to consumer_thread_extent automatically.
 *
 * Default: 128 (backward compatible).
 *
 * kFineGrainedWsProducerThreadExtent = "tl.finegrainedws_producer_thread_extent"
 */
static constexpr const char *kFineGrainedWsProducerThreadExtent =
    "tl.finegrainedws_producer_thread_extent";

/*!
 * \brief Enable three-role warp specialization (producer + dQ writer + consumer)
 *
 * When enabled, FineGrainedWS detects TMA reduce-add patterns in the consumer loop
 * (from lowered T.atomic_add with use_tma) and extracts them into a dedicated
 * dQ writer warp within the producer warp group, following FA3's design.
 *
 * Default: false
 *
 * kFineGrainedWsEnableThreeRole = "tl.finegrainedws_enable_three_role"
 */
static constexpr const char *kFineGrainedWsEnableThreeRole =
    "tl.finegrainedws_enable_three_role";

/*!
 * \brief Enable dual-consumer warp group mode for TC pipeline overlap.
 * Splits consumer into WG0 (QK+Softmax) and WG1 (PV).
 * Default: false
 */
static constexpr const char *kFineGrainedWsDualConsumer = "tl.finegrainedws_dual_consumer";

/*!
 * \brief Per-buffer consumer stage offsets for cross-iteration overlap.
 *
 * Semicolon-separated buffer entries: "buffer:pattern=offset"
 * Example: "V_shared:wgmma=-1"
 * Enables QK(k)||PV(k-1) overlap in FlashAttention.
 *
 * Default: "" (empty, all consumers use offset=0)
 */
static constexpr const char *kFineGrainedWsConsumerStageMap =
    "tl.finegrainedws_consumer_stage_map";

/*!
 * \brief Phase B barrier position hints for FineGrainedWS.
 *
 * Semicolon-separated entries: "buffer:wait=W,arrive=A"
 * Example: "K_shared:wait=1,arrive=4"
 * Overrides default first_read/last_access+1 barrier placement.
 *
 * Default: "" (empty, use default placement)
 */
static constexpr const char *kFineGrainedWsBarrierHints = "tl.finegrainedws_barrier_hints";

/*!
 * \brief Phase B auto-derived per-compute-stmt stage offsets.
 *
 * Comma-separated entries: "idx=offset"
 * Example: "3=-1"
 *
 * Default: "" (empty)
 */
 ...
 * with dynamic distribution via atomicAdd.
 *
 * Default: false
 */
static constexpr const char *kPersistentKernel = "tl.persistent_kernel";

/*!
 * \brief Number of SMs for persistent kernel (0 = auto-detect).
 */
static constexpr const char *kPersistentNumSms = "tl.persistent_num_sms";

/*!
 * \brief Enable L2-friendly tile swizzling in persistent kernel.
 */
static constexpr const char *kPersistentL2Swizzle = "tl.persistent_l2_swizzle";
...
/*!
 * \brief Named barrier arrive (non-blocking signal)
 *
 * named_barrier_arrive(barrier_id, thread_count)
 * PTX: bar.arrive barrier_id, thread_count
 *
 */
TVM_DLL const Op &named_barrier_arrive();

/*!
 * \brief Named barrier wait (blocking arrive + wait)
 *
 * named_barrier_wait(barrier_id, thread_count)
 * PTX: bar.sync barrier_id, thread_count
 *
 */
TVM_DLL const Op &named_barrier_wait();
