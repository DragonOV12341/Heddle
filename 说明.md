# 关于SMT求解输入
```
[d] ---- func before SMT ----- # from tvm.script import tir as T
...
def main(Q_handle: T.handle, K__handle: T.handle, V_handle: T.handle, O_handle: T.handle):
    ...
    T.create_list_of_mbarrier(1, 1, 1, 1, 1, 1, 1)
    with T.block("root"):
        T.reads()
        T.writes()
        ...
        with T.block("tilelang_root"):
            ...
            Ks = T.alloc_buffer((2, 8, 512), "float16", data=Ks.data, scope="shared.dyn")
            Vs = T.alloc_buffer((2, 8, 512), "float16", data=Vs.data, scope="shared.dyn")
            ...
            for k in T.serial(64, annotations={"num_stages": 3}):
                with T.attr(Ks.data, "tl.tma_copy_write_buffer", 1):
                    if tx == 0:
                        T.mbarrier_expect_tx(T.get_mbarrier(k % 3 + 1), 16384)
                    if tx == 0:
                        for i in T.unroll(2):
                            T.tma_load(T.create_tma_descriptor(6, 4, K_.data, 128, 32, 4096, 1, T.int64(2), T.int64(256), T.int64(8192), T.int64(33554432), 64, 1, 64, 1, 1, 1, 1, 1, 0, 3, 2, 0), T.get_mbarrier(k % 3 + 1), T.tvm_access_ptr(T.type_annotation("float16"), Ks.data, i * 4096, 4096, 2), i * 64, by, k * 64, 0, 0)
                    if tx == 0:
                        T.ptx_arrive_barrier(T.get_mbarrier(k % 3 + 1))
                T.mbarrier_wait_parity(T.get_mbarrier(k % 3 + 1), k % 6 // 3)
                for i in T.vectorized(4):
                    smp[i] = sm[i]
                for i in T.vectorized(4):
                    sm[i] = T.infinity("float32") * T.float32(-1.0)
                ...
                with T.block("_gemm_ssr"):
                    ...
                    T.initialize_wgmma_descriptor(desc_b[0], T.tvm_access_ptr(T.type_annotation("float16"), Ks.data, 0, 8192, 1), 1, 1, 64)
                ...
                with T.attr(Vs.data, "tl.tma_copy_write_buffer", 1):
                    if tx == 0:
                        T.mbarrier_expect_tx(T.get_mbarrier(k % 3 + 4), 16384)
                    if tx == 0:
                        for i in T.unroll(2):
                            T.tma_load(T.create_tma_descriptor(6, 4, V.data, 128, 32, 4096, 1, T.int64(2), T.int64(256), T.int64(8192), T.int64(33554432), 64, 1, 64, 1, 1, 1, 1, 1, 0, 3, 2, 0), T.get_mbarrier(k % 3 + 4), T.tvm_access_ptr(T.type_annotation("float16"), Vs.data, i * 4096, 4096, 2), i * 64, by, k * 64, 0, 0)
                    if tx == 0:
                        T.ptx_arrive_barrier(T.get_mbarrier(k % 3 + 4))
                T.mbarrier_wait_parity(T.get_mbarrier(k % 3 + 4), k % 6 // 3)
                with T.block("_gemm_rsr"):
                    T.reads()
                    T.writes()
                    desc_b = T.alloc_buffer((1,), "uint64", scope="local.descriptor.wgmma")
                    T.initialize_wgmma_descriptor(desc_b[0], T.tvm_access_ptr(T.type_annotation("float16"), Vs.data, 0, 8192, 1), 1, 512, 64)
...
```
SMT求解开始，输入的IR如上。可见：
1. 代码里的寄存器数目是per-thread的
2. shm的数目按照block进行

所以进入 joint 求解后，加约束检查 reg和shm用量时：

- 每个warp的reg用量不超过 reg_limit : 根据1，对warp求和, 求得的也是 warp[i] 内某个线程的寄存器数目。该数值只要小于 reg_limit 即可 
- 每个block的shm用量 ： 直接对IR上的shm做统计即可。
